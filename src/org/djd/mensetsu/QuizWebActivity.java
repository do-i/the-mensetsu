...
import static org.djd.mensetsu.global.ApplicationCommons.DEFAULT_WEB_FONT_SIZE;
...
import org.djd.mensetsu.global.StringUtil;
...
public class QuizWebActivity extends Activity {
	...
	private static final String TAG = QuizWebActivity.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.quiz_web_view);
		questionIdView = (TextView) findViewById(R.id.id_question_id);
		qaCountView = (TextView) findViewById(R.id.id_qa_count);

		questionWebView = (WebView) findViewById(R.id.id_web_view_question);
		questionWebView.getSettings().setBuiltInZoomControls(true);
		questionWebView.setBackgroundColor(Color.parseColor(getString(R.color.light_green)));

		answerWebView = (WebView) findViewById(R.id.id_web_view_answer);
		answerWebView.getSettings().setBuiltInZoomControls(true);
		answerWebView.setBackgroundColor(Color.parseColor(getString(R.color.light_blue)));

		quizBroadcastReceiver = new QuizBroadcastReceiver();
		registerReceiver(quizBroadcastReceiver, quizBroadcastReceiver.intentFilter);

		boolean continueFlag = getIntent().getBooleanExtra(INTENT_EXTRA_KEY_CONTINUE, false);
		if (continueFlag) {
	...
	private void loadStateFromSharedPreferences() {
		...
		questionAnswerIdsIndex = sharedPreferences.getInt(SAVE_QUESTION_CURR_IDX_KEY, 0);
		answerVisible = sharedPreferences.getBoolean(SAVE_ANSWER_VISIBLE_KEY, false);
		questionAnswerIds = StringUtil.unmarshall(sharedPreferences.getString(SAVE_QUESTION_LIST_KEY, null));
		if (answerVisible) {

			answerWebView.setVisibility(View.VISIBLE);

			ToggleButton btn = (ToggleButton) findViewById(R.id.button_answer);
			btn.setChecked(true);

		} else {

			answerWebView.setVisibility(View.INVISIBLE);
		}
	}
	...
	protected void onStart() {
		...
		SharedPreferences sharedPreferences = getSharedPreferences(PREFERENCE_FILE_NAME, MODE_PRIVATE);
		final int FONT_SIZE = sharedPreferences.getInt(PREFERENCE_KEY_WEB_FONT_SIZE, DEFAULT_WEB_FONT_SIZE);

		questionWebView.getSettings().setDefaultFontSize(FONT_SIZE);
		answerWebView.getSettings().setDefaultFontSize(FONT_SIZE);
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		if (null != savedInstanceState) {
			...
			answerVisible = savedInstanceState.getBoolean(SAVE_ANSWER_VISIBLE_KEY);
			if (answerVisible) {
	...
	public void onButtonClicked(View view) {
		...
		case R.id.button_prev:
			...
			if (questionAnswerIdsIndex <= 0) {
				Toast.makeText(QuizWebActivity.this, R.string.msg_at_first_question, Toast.LENGTH_SHORT).show();
			} else {
		...
		case R.id.button_next:
			if (questionAnswerIdsIndex >= questionAnswerIds.size() - 1) {
				Toast.makeText(QuizWebActivity.this, R.string.msg_at_last_question, Toast.LENGTH_SHORT).show();
			} else {
				++questionAnswerIdsIndex;
				display();
	...
	protected void onPause() {
		...
		editor.putInt(SAVE_QUESTION_CURR_IDX_KEY, questionAnswerIdsIndex);
		editor.putBoolean(SAVE_ANSWER_VISIBLE_KEY, answerVisible);
		editor.putString(SAVE_QUESTION_LIST_KEY, StringUtil.marshall(questionAnswerIds));
		editor.commit();
	...
	private class CategoriesOnMultiChoiceClickListener implements OnMultiChoiceClickListener {
		...
		public void onClick(DialogInterface dialogInterface, int possition, boolean isChecked) {
			...
			if (0 == updatedRowCount) {
				...
				Toast.makeText(QuizWebActivity.this, "Failed to update the change.", Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	private void display() {
		...
		if (0 != questionAnswerIds.size()) {
			...
			cursor = managedQuery(CategoryContentProvider.CONTENT_URI, CategoryEntity.Columns.FULL_PROJECTION,
			      CategoryEntity.Columns._ID + "=?", new String[] { String.valueOf(questionAnswerEntity.categoryId) },
			      null);
			CategoryEntity categoryEntity = CategoryEntity.Helper.convertToCategoryEntity(cursor, 0);

			questionIdView.setText(String.format("Question ID: %d (%s)", questionAnswerEntity.questionId,
		...
		questionWebView.scrollTo(0, 0);
		questionWebView.loadData(question, TEXT_HTML, UTF_8);
		answerWebView.scrollTo(0, 0);
		answerWebView.loadData(answer, TEXT_HTML, UTF_8);
	...
	public class QuizBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (null != errorMsg) {
				Toast.makeText(QuizWebActivity.this, R.string.msg_shuffle_error, Toast.LENGTH_SHORT).show();

			} else {
				Log.i(TAG, "Shuffle completed.");
				questionAnswerIds = null;
				Toast.makeText(QuizWebActivity.this, R.string.msg_shuffle_completed, Toast.LENGTH_SHORT).show();
